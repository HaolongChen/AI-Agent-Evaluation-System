...
import { goldenSetService } from "../../applications/usecases/goldenset.usecase.ts";
import { projectService } from "../../applications/usecases/project.usecase.ts";
    ...
    getGoldenSets: async (
      ...
      try {
        ...
        const result = await goldenSetService.getGoldenSets({
          ...arguments_.filters,
          copilotType: arguments_.filters.copilotType
            ? COPILOT_TYPES[arguments_.filters.copilotType]
            : undefined,
        });
    ...
    initializeGoldenSet: async (
      ...
      try {
        const result = await goldenSetService.createGoldenSet({
          ...arguments_.input,
          copilotType: COPILOT_TYPES[arguments_.input.copilotType],
        });
        return {
          ...result,
          copilotType: REVERSE_COPILOT_TYPES[
