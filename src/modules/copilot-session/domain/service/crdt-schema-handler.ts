...
export class CrdtSchemaHandler {
	...
	private schemaIdGetter = (copilotInputAggregate: CopilotInputAggregate) => {
		return copilotInputAggregate.getEntity("goldenSet").getData("schemaId");
	};
	...
	async getSchemaGraph(
		...
		const schemaId = this.schemaIdGetter(copilotInputAggregate);
		const arrayBuffer =
