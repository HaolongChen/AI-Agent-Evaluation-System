...
import type { ProjectActivatedEvent } from "../../domain/event/project-activated.event.ts";
import type { ProjectCreatedEvent } from "../../domain/event/project-created.event.ts";
import type { ICopilotExecutionPool } from "../../domain/interface/copilot-execution-pool.ts";
...
export class ProjectHandler {
	constructor(
		...
		private readonly copilotExecutionPool: ICopilotExecutionPool,
	) {}
	...
	async onProjectActivated(event: ProjectActivatedEvent) {
		return this.copilotExecutionPool.publish(
			event.project.getData("id"),
			event.project,
		);
	}
}
