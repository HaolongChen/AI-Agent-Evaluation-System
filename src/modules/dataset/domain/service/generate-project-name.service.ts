...
export class ProjectNameService {
  ...
  generateProjectName(): string {
    ...
    this.lastProjectName = this.truncate(
      `temp-project-${goldenSetId[0]}-${userInputId[0]}-${Date.now()}`,
    );
  ...
  retry(): string {
    if (this.maxRetry <= 0) {
      throw new Error(
        "Maximum retry attempts reached for project name generation.",
      );
    }
    if (!this.lastProjectName) {
      throw new Error(
        "No project name generated yet. Please call generateProjectName() first.",
      );
    }
    this.lastProjectName = this.truncate(
      `${this.truncate(this.lastProjectName, this.lastProjectName.length / 4)}-retry-${Date.now()}`,
    );
    this.maxRetry--;
    return this.lastProjectName;
  }
