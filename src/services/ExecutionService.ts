...
export class ExecutionService {
	async linkCopilotOutputToQuestionSetById(
		copilotOutputId: number,
		questionSetId: number,
	) {
		try {
			const copilotOutput = await prisma.copilotOutput.findUnique({
				where: { id: copilotOutputId },
			});

			if (!copilotOutput) {
				throw new Error(`Copilot output with ID ${copilotOutputId} not found`);
			}
			...
			if (copilotOutput.userInputId !== questionSet.userInputId) {
				throw new Error(
					`User input ID mismatch: Copilot output has userInputId ${copilotOutput.userInputId} but question set has userInputId ${questionSet.userInputId}`,
				);
			}

			await prisma.questionSet.update({
				where: { id: questionSetId },
				data: { userInputId: copilotOutput.userInputId },
			});
		} catch (error) {
			logger.error("Error linking copilot output to question set:", error);
			throw new Error("Failed to link copilot output to question set");
		}
	}
